<?php
namespace Net\TomasKadlec\LunchGuy\BaseBundle\Service\Output;
...
/**
 * Class Slack
 * @package Net\TomasKadlec\LunchGuy\BaseBundle\Service\Output
 */
class Slack implements OutputInterface
{
    ...
    public function send($format, $restaurantId, $menu, $options = [])
    {
        ...
        if (!isset($options['uri']) || !isset($options['username']) || !isset($options['channel']))
            throw new \RuntimeException("Options lunch_guy.output.slack.{uri,username,channel} are required.");
